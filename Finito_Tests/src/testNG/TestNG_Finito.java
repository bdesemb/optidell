...
public class TestNG_Finito {
	...
	private static String[] emailJoueurs = { "emmeline.leconte@ipl.be",
			"michel.debacker@ipl.be", "olivier.choquet@ipl.be" };
	...
	public void testInscriptionJoueurs() {
		for (int i = 0; i < nomsJoueurs.length; i++) {
			joueurs[i] = gestionJoueurs.inscription(nomsJoueurs[i],
					prenomsJoueurs[i], emailJoueurs[i], loginsJoueurs[i],
					motDePasseJoueurs[i]);
		}
		...
		for (int i = 0; i < nomsJoueurs.length; i++) {
			...
			assertEquals(motDePasseJoueurs[i], listeJoueurs.get(i)
					.getMotDePasse());
	...
	public void testConnexionJoueur() {
		for (int i = 0; i < nomsJoueurs.length; i++) {
			assertEquals(gestionJoueurs.connexion(loginsJoueurs[i],
					motDePasseJoueurs[i]), joueurs[i]);
		}
	}
	...
	public void testRejoindrePartie() {
		...
		assertEquals(gestionParties.listerPartiesEnAttente().get(0)
				.getPlateauEnJeu().get(1).getJoueur(), joueurs[1]);
	}
	...
	public void testDebuterPartie() {
		...
		partieEnCours = gestionParties.recupererPartieAvecID(partieEnCours
				.getId());
		assertEquals(true, partieEnCours.isEnCours());
		assertEquals(partieEnCours.getJetonsRestants().size(), 12);
		assertEquals(partieEnCours.getIndiceTirage(), 3);
		assertEquals(
				3,
				gestionPlateaux.recupererMainPlateau(
						partieEnCours.getPlateauEnJeu().get(0)).size());
	}

	@Test(priority = 6)
	public void testJouerPionSurUnPlateau() {
		// int lancéDeDé = partieEnCours.getResultatDe();
		Plateau p = partieEnCours.getPlateauEnJeu().get(1);
		List<Jeton> main = gestionPlateaux.recupererMainPlateau(partieEnCours
				.getPlateauEnJeu().get(0));
		List<Case> casesDuPlateau = gestionPlateaux.recuperLaListeDeCase(p);
		assertEquals(
				true,
				gestionPlateaux.placerJeton(p, main.get(0),
						casesDuPlateau.get(20)));
		assertEquals(main.get(0),
				gestionPlateaux.recuperLaListeDeCase(p).get(20).getJeton());
	}
	
	@Test(priority = 7)
	public void testDeplacerPionSurUnPlateau() {
